...
test('stamp sync', async function (t) {
  ...
  const locals = { name: 'world', version: 'v.1.2.3', count: 100, url: 'https://docs.pears.com/', arr: ['a', 0] }
...
test('stamp stream w/ promise local', async function (t) {
  ...
  const locals = { name: 'world', version: Promise.resolve('v.1.2.3'), count: 100, url: 'https://docs.pears.com/', arr: ['a', 0] }
  const stream = stamp.stream(template, locals)
  t.teardown(() => stream.destroy())
...
test('stamp stream w/ stream local', async function (t) {
  ...
  const locals = { name: 'world', version: Readable.from(['v.1.2.3']), count: 100, url: 'https://docs.pears.com/', arr: ['a', 0] }
  const stream = stamp.stream(template, locals)
  t.teardown(() => stream.destroy())
